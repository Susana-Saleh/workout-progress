...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const snap = await db.collection('nutrition_logs').where('user_id', '==', uid).get();
    ...
    if (req.method === 'POST') {
      const body = req.body;
      const doc = await db.collection('nutrition_logs').add({ ...body, user_id: uid });
    ...
    if (req.method === 'DELETE') {
      ...
      const ref = db.collection('nutrition_logs').doc(id);
    ...
    console.error('nutrition_logs error:', e);
